...
void calc_r(double *xi, double *yi, double *zi, double *r){
    int r_cnt = 0;
...
int cluster_check(double *x, double *y, double *z, struct Cluster *clustersp){
    ...
    void neighbor_list(double *r, int *cntlist, int rlen){
        int ii = 0;
        int jj = ii+1;
    ...
    neighbor_list(r, cntlistp, r_length); // finding the neighbors. populates the upper triangle only
    for (int i=0; i<n_mols; i++){
        for (int j=0; j<n_mols; j++){
            if (i != j){
