...
void init_pos_mono(double *x, double *y, double *z, int mono_id){
    clock_t t_b = clock(); 
    gsl_rng *randgen = gsl_rng_alloc(gsl_rng_taus);
    gsl_rng_set(randgen, (double)t_b * mono_id);
    double xc;
    double yc;
    double zc;
    double dirx;
...
int cluster_check(double *x, double *y, double *z, struct Cluster *clustersp){
    ...
    int no_clusters = cluster_list(cntlistp, clustersp);
    if (no_clusters > 1){
        return no_clusters;
    }
    return 0;
...
void break_clusters(double *x, double *y, double *z, struct Cluster *clusters, int n_clusters){
...
int fib_size(struct Cluster *clusters, int n_clusters){
    int max_size = clusters[0].size;
    for (int i=0; i<n_clusters; i++){ 
        if (max_size < clusters[i].size){
            max_size = clusters[i].size;
